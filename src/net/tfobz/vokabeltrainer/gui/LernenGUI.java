...
public class LernenGUI extends JDialog {
	...
	public LernenGUI(JFrame owner, int nummerLernkartei, int[] nummernFaecher) {
		...
		btnWeiter.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (VokabeltrainerDB.getKarte(numKarten.get(numKarteCurrent))
						.getRichtig((textField.isEnabled()) ? textField.getText() : textField_1.getText())) {
					...
					//Richtig oder Falsch wird kurzzeitig dem Benutzer gezeigt
					//bevor dieser erneute Möglichkeiten bekommt
	...
	private void updateView() {
		...
		if (this.numKarten == null) {
			if (this.numFachCurrent + 1 == this.numFaecher.length) {
				JOptionPane.showMessageDialog(ownerFrame, "Sie haben das Training abgeschlossen!", "Ende",
			...
			for (int i = 0; i < length; i++) {
				int num = -1;
				do {
					num = VokabeltrainerDB.getZufaelligeKarte(this.lnummer, this.numFaecher[numFachCurrent])
							.getNummer();
